...
def get_vit2k(pretrained_weights, arch='vit2k_xs', device=torch.device('cuda:1')):
    r"""
    Builds ViT-2K Model.
    
    Args:
    - pretrained_weights (str): Path to ViT-2K Model Checkpoint.
    - arch (str): Which model architecture.
    - device (torch): Torch device to save model.
    
    Returns:
    - model2k (torch.nn): Initialized model.
    """
    
    checkpoint_key = 'teacher'
    #device = torch.device("cuda:1") if torch.cuda.is_available() else torch.device("cpu")
    model2k = vits.__dict__[arch](num_classes=0)
    for p in model2k.parameters():
        p.requires_grad = False
    model2k.eval()
    model2k.to(device)
    ...
    if os.path.isfile(pretrained_weights):
        ...
        # remove `backbone.` prefix induced by multicrop wrapper
        state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
        msg = model2k.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(pretrained_weights, msg))
